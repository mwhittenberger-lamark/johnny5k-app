...
class IronQuestAiNarrativeService {
	...
	public static function build_transition_bundle( int $user_id, array $profile, array $location, array $mission, array $state, array $current_exercise, array $next_encounter ): array {
		$story_engine_draft = is_array( $next_encounter['story_engine_draft'] ?? null ) ? $next_encounter['story_engine_draft'] : [];
		$story_engine_template = is_array( $next_encounter['story_engine_template'] ?? null ) ? $next_encounter['story_engine_template'] : [];
		...
		if ( [] !== $story_engine_draft || [] !== $story_engine_template ) {
			$transition_payload['story_engine'] = self::build_story_engine_payload_branch( $story_engine_draft, $story_engine_template );
		}
	...
	private static function build_set_progression_payload( array $profile, array $location, array $mission, array $state, array $current_exercise, array $beat_context, string $raw_set_result ): array {
		$anchor = is_array( $location['ai_prompt_anchor'] ?? null ) ? $location['ai_prompt_anchor'] : [];
		$story_engine_draft = is_array( $current_exercise['story_engine_draft'] ?? null ) ? $current_exercise['story_engine_draft'] : [];
		$story_engine_template = is_array( $current_exercise['story_engine_template'] ?? null ) ? $current_exercise['story_engine_template'] : [];
		...
		$payload = [
			...
			'encounter_seed' => self::filter_story_struct_keys(
				self::normalize_encounter_seed_for_prompt( (array) ( $state['encounter_seed'] ?? [] ) ),
				[ 'title', 'objective', 'threat', 'prop', 'landmark', 'hazard', 'stakes', 'enemy_posture', 'sensory_detail', 'scene_brief', 'player_goal', 'opponent_pressure', 'failure_cost', 'setting_detail' ]
			),
		...
		if ( [] !== $story_engine_draft || [] !== $story_engine_template ) {
			$payload['story_engine'] = self::build_story_engine_payload_branch( $story_engine_draft, $story_engine_template );
		}
