...
export default function JohnnyHomeScreen() {
  ...
  const [dailyBriefingActionDismissed, setDailyBriefingActionDismissed] = useState(false)
  ...
  const clearDailyBriefingAction = useCallback(() => {
    setDailyBriefingActionDismissed(true)
    setNavIndex(0)
  }, [])
  ...
  const actionSlides = [
    ...
    ...(!dailyBriefingActionDismissed ? [{
      key: 'checkin',
      title: 'Today’s Briefing',
      subtitle: 'Readiness · progress · today’s plan',
      onClick: openDailyBriefing,
      onClear: clearDailyBriefingAction,
      clearLabel: 'Clear daily briefing action',
      secondary: true,
    }] : []),
    {
      key: 'progress-log',
      title: 'Log Progress',
      subtitle: 'Sleep · weight · steps · photos',
    ...
    },
    {
      ...
      title: 'Log Nutrition',
      subtitle: 'Food · water',
      onClick: openNutritionLog,
      secondary: true,
    },
    {
      key: 'diary',
      title: 'Progress Diary',
      subtitle: 'Daily log · trends · meals',
      onClick: openProgressDiary,
      secondary: true,
    },
    {
      key: 'profile',
      title: 'Profile & Settings',
      subtitle: 'Goals · units · reminders',
      onClick: openProfile,
      secondary: true,
    },
  ]
  const maxNavIndex = Math.max(0, actionSlides.length - 1)
      ...
      <div className="johnny-prototype-phone" style={{ '--johnny-primary-dock-height': `${primaryDockHeight}px`, '--johnny-accent-rgb': resolveAccentRgb(accentColor) }}>
        ...
        <nav className="nav-carousel" aria-label="Johnny actions">
          <div className="nav-viewport">
            <div
              ...
              onPointerUp={event => {
                ...
                if (distance < -40) setNavIndex(current => Math.min(maxNavIndex, current + 1))
                navStartXRef.current = null
              }}
            >
              {actionSlides.map(slide => (
                <ActionSlide
                  key={slide.key}
                  title={slide.title}
                  ...
                  onLongPress={slide.onLongPress}
                  disabled={slide.disabled}
                  suggestionLabel={slide.suggestionLabel}
                  suggestionPhase={slide.suggestionLabel ? suggestionPhase : 'idle'}
                  onClear={slide.onClear}
                  clearLabel={slide.clearLabel}
                />
              ))}
            </div>
          </div>
          <div className="nav-controls">
            ...
            <div className="nav-dots">{actionSlides.map((slide, index) => <span key={slide.key} className={`nav-dot${index === navIndex ? ' active' : ''}`} />)}</div>
            <button type="button" className="nav-arrow" onClick={() => setNavIndex(Math.min(maxNavIndex, navIndex + 1))} aria-label="Next action">›</button>
          </div>
        </nav>
...
function ActionSlide({ title, subtitle, secondary = false, onClick, onLongPress, onClear, clearLabel = 'Clear action', disabled = false, suggestionLabel = '', suggestionPhase = 'idle' }) {
  ...
  return <div className={`cta-slide${onClear ? ' has-clear-action' : ''}`}><button
    type="button"
    className={`activate-btn${secondary ? ' secondary-cta' : ''}${pressing ? ' long-pressing' : ''}${suggestionLabel ? ` suggestion-${suggestionPhase}` : ''}`}
  ...
  >{suggestionLabel ? <span className="activate-suggestion-label" aria-live="polite">{suggestionLabel}</span> : null}<span className="activate-title">{title}</span><span className="activate-sub">{subtitle}</span></button>{onClear ? <button type="button" className="activate-clear" onClick={onClear} aria-label={clearLabel}>Clear</button> : null}</div>
}
